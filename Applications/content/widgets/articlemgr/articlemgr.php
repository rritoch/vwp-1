...
/**
 * Content Article Manager Widget
 *  
 * @package    VWP.Content
 * @subpackage Widgets
...
class Content_Widget_Articlemgr extends VWidget 
{
    ...
    function delete_articles($tpl = null) 
    {
        ...
        if (count($selected) < 1) {
            ...
            if (!VWP::isWarning($articles)) {
                ...
                VWP::addNotice("Articles deleted!");
            }
        }
        $this->display($tpl);
    }
    ...
    function edit_article($tpl = null) 
    {
        ...
        if (VWP::isWarning($article)) {
            ...
            $image_list_url = 'index.php?app=content&widget=images&format=xml&mode=raw';
            $image_list_url = VRoute::getInstance()->encode($image_list_url);
            
            $eargs = array('image_list_url'=>$image_list_url);
                        
            $editor = $article->getEditor($eargs);                                    
            $article = $article->getProperties();
        }
    ...
    // end class Content_Widget_Articlemgr    
}
